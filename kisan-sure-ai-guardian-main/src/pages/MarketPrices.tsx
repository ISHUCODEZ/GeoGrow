...
const MarketPrices: React.FC = () => {
      ...
      try {
        ...
        if (statesData.length > 0) {
          ...
          const [districtsData, commoditiesData] = await Promise.all([
            ...
            marketService.getCommoditiesByState(defaultState)
          ]);
  ...
  useEffect(() => {
    if (state) {
      ...
      marketService.getCommoditiesByState(state).then(commodities => {
  ...
  useEffect(() => {
    if (state) {
      marketService.getCommoditiesByState(state, district, market).then(commodities => {
  ...
  useEffect(() => {
    if (state) {
      marketService.getCommoditiesByState(state, district, market, commoditySearch).then(setCommodities);
    }
